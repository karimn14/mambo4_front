  ...
  >
    <div class="flex justify-between">
      <div>
        ...
        <p class="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
          Penerima Manfaat Berdasarkan Provinsi
        </p>
      </div>
    </div>
    <div
    ...
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="items-center w-full rounded-full max-w-8">
            <img src="/images/country/sumbar.svg" alt="Sumbar" />
          </div>
          <div>
            <p class="font-semibold text-gray-800 text-theme-sm dark:text-white/90">Sumatera Barat</p>
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="items-center w-full rounded-full max-w-8">
            <img src="/images/country/jabar.svg" alt="Jabar" />
          </div>
          <div>
            <p class="font-semibold text-gray-800 text-theme-sm dark:text-white/90">Jawa Barat</p>
            <span class="block text-gray-500 text-theme-xs dark:text-gray-400">
              3,589 Siswa
            </span>
          </div>
        ...
        <div class="flex w-full max-w-[140px] items-center gap-3">
          <div class="relative block h-2 w-full max-w-[100px] rounded-sm bg-gray-200 dark:bg-gray-800">
            <div
              class="absolute left-0 top-0 flex h-full w-[53%] items-center justify-center rounded-sm bg-brand-500 text-xs font-medium text-white"
            ></div>
          </div>
          <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">53%</p>
        </div>
      </div>
    </div>
  </div>
...
// Define a type for jsVectorMap
type JsVectorMapType = {
  new (options: any): any
}

const mapOneRef = ref<HTMLElement | null>(null)
const mapInstance = ref<InstanceType<JsVectorMapType> | null>(null)

const initMap = () => {
  if (mapOneRef.value) {
    mapInstance.value = new (jsVectorMap as JsVectorMapType)({
      ...
      onRegionTooltipShow: function (event: MouseEvent, tooltip: { setContent: (content: string) => void; text: () => string }) {
